...
from legacy.economics.economic_class_sp import EnergyLettuceEconomics
...
class IntegratedSimulation:
    def __init__(self, DLI, design_para, new_eval_p, pv_power_traj, wind_power_traj, electricity_prices, period_codes):
        # print("DEBUG IntegratedSimulation.period_codes:", type(period_codes), 
    ...
    def run(self):
        ...
        try:
            while m < N - 1:
                ...
                # # Grid-only case check
                # is_grid_only = (
                #     P_pv == 0 and
                #     P_wind == 0 and
                #     self.batt_param.get("battery_capacity", 0) == 0
                # )

                # if is_grid_only:  # this is the baseline without any renewable energy or battery
                #     # No battery or RE → full load from grid
                #     SoC = 0
                #     P_battery = 0
                #     P_grid_ex = 0
                #     P_grid_in = min(P_load, self.batt_param.get("P_grid_max_import", float('inf')))
                #     P_deficit = max(0, P_load - P_grid_in)
                #     P_excess = 0
                #     P_renewable_to_battery[m] = 0
                #     P_battery_to_load[m] = 0
                #     P_direct_renewable_to_load[m] = 0
                ...
                if no_storage:
                    ...
                    # Only reset initial SoC, don't reset entire array
                    SoC = 0
                    P_battery = 0
                    P_renewable_to_battery[m] = 0
                    P_battery_to_load[m] = 0
                    ...
                    if net >= 0:
                        ...
                        self.electricity_prices_updated[m] = self.electricity_prices_updated[m] * 1.05 *1.21 + 0.00  # or 0.5, etc.
                    else:
                        # Deficit → import up to limit, then leave unmet
                        need       = -net
                        P_grid_in  = min(need, self.batt_param.get("P_grid_max_import", float("inf")))
                        self.electricity_prices_updated[m] = (self.electricity_prices_updated[m] * 1.05)*1.21  # normal price for import
                    ...
                    if current_net_power >= 0:  
                        ...
                        self.electricity_prices_updated[m] = self.electricity_prices_updated[m] * 1.05 *1.21 + 0.00
                        if P_grid_ex < 0:
        ...
        # Check if there is any positive export
        # print("Any positive export?", np.any(self.P_grid_ex_history > 0))
        ...
        # save the electricity_prices_updated and electricity_prices for comparison in the same excel file
        prices_df = pd.DataFrame({
            "Original Prices": self.electricity_prices,
            "Updated Prices": self.electricity_price_history.flatten()
        })
        # prices_df.to_excel("electricity_prices_comparison_2.xlsx", index=False)
        
        # print("DEBUG self.period_codes before econ:", type(self.period_codes),
        #       None if self.period_codes is None else self.period_codes.shape)
    ...
    def _final_metrics(self, harvest_dw_arr):
        ...
        # toll_info = self.econ.grid_tolls_spain()
        # period_cost = toll_info["total_tolls_eur"]   # you can add to existing electricity cost term
        net_cost = self.econ.grid_operational_cost() + self.econ.grid_tolls_spain()
        # self.econ.sanity_check_grid_signs()
        ...
        results = {
            ...
            "Electricity generation per year (kWh)": round(float(np.sum(self.E_re_generation)), 4),
            "Electricity used by load per year (kWh)": round(float(Total_e_load), 4),
            "Electricity provided by battery to the load per year (kWh)": round(float(np.sum(self.E_batt_to_load)), 4),
            ...
            "Curtailment ratio": round(float(curtail_ratio), 4),
            "Renewable Fraction": round(float(re_fraction), 4),            # checked and updated to net import from grid
            "Self-consumption ratio": round(float(self_consum_rate), 4),      # checked
            "Self-sufficiency ratio": round(float(self_suf_ratio), 4),      # checked, 
            ...
            "Grid Net Cost with tax (SP) (€/yr)": round(float(net_cost), 4),                #checked
...
if __name__ == "__main__":  
    ...
    design_para["num_pv"] =3988 #4000 #5500#5000
    design_para["num_wind"] = 0 #1 3#8#8
    design_para["num_light"] = 14812+5*741 #+741*9#21482 # 6670
    ...
    new_eval_p = {}
    new_eval_p["pv_unit_cost"] = 810*0.95*0.34  #810$/kw=692euro,2021 # 350# 140*2.5
    ...
    new_eval_p["lettuce_price"] = 1.08*1.0
    new_eval_p["co2_emission_factor"] = 0.174 # 0.329 #0.329 #0.329 # 0.174 # 0.329 # netherlands 2021: 0.329 tonsco2/MWh
